...
namespace MyPacketCapturer
{
    public partial class frmCapture : Form
    {
       ...
       static frmSend fSend;  //This will be our send form
       static Boolean  ackConnection;
        ...
        private static void device_OnPacketArrival(object sender, CaptureEventArgs packet)
        {
            ...
            if (data[23] == 6)
            {
                if (data[26] == fSend.ipaddressListen[0])
                {
                    if (data[27] == fSend.ipaddressListen[1])
                    {
                        if (data[28] == fSend.ipaddressListen[2])
                        {
                            if (data[29] == fSend.ipaddressListen[3])
                            {
